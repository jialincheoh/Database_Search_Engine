...
# print zmat
def write_zmat(filename, geom, bond_graph):
    at_types = geom[0]
    n_atoms = len(at_types)
    with open(filename, 'w') as outfile:
        for i in range(n_atoms):
            outfile.write('%s' % (at_types[i]))
            for j in range(len(bond_graph[i])):
                if bond_graph[i][j] < i:
                    outfile.write('\t%i' % (bond_graph[i][j] + 1))
            outfile.write('\n')
        outfile.write('\n')
...
if __name__ == "__main__":
    ...
    print_bond_graph(geom, bond_graph, 'hi')
    #print_angles(geom, angles)

    write_zmat(xyz_file_name.replace('.xyz', '.zmat'), geom, bond_graph)
